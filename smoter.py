...
def smoter(
    ...
    manualPercentage = None   ## manual percentage should be a dictionary with length of 2. 
    ...
    """
    ## go through quality checks for input arguments: 
    if (rel_method == "manual" and manualPercentage is None and samp_method == "balance"): 
        raise ValueError("Cannot proceed: manualPercentage is required for manual method")
    
    if (rel_method == "manual" and len(manualPercentage) != 2 and samp_method == "balance"):
        raise ValueError("Cannot proceed: manualPercentage must be a dictionary with length of 2")
    ...
    for i in range(0, len(y_sort) - 1):
        if ((y_phi[i] >= rel_thres and y_phi[i + 1] < rel_thres) or 
            (y_phi[i] < rel_thres and y_phi[i + 1] >= rel_thres)):
                bumps.append(i + 1) # bump classification through indicies. 
    ...
    for i in range(n_bumps):
        b_index.update({i: y_sort[bumps[i]:bumps[i + 1]]}) # bumps collects the range index between our normal/rare bumps. 
        index = bumps[i+1] - 1
        if (y_phi[index] >= rel_thres): 
            bumpClassifcation.append("rare") # if it's greater than the rel_thers
        if (y_phi[index] < rel_thres): 
            bumpClassifcation.append("normal") # if it's greater than the normal threshold. 
                                
    print("bumpClassification index 0:", bumpClassifcation[0])
    ...
    if samp_method == "balance":
        ...
        for i in bumpClassifcation: 
            if i == "rare":
                s_perc2.append(manualPercentage["rare"])
            else:
                s_perc2.append(manualPercentage["normal"]) 
    ...
    for i in range(n_bumps):
        ...
        if under_samp is True:
            if s_perc[i] < 1:
                ...
                ### omit_obs = data 
                ###-> where data should be focused on the BIN subregion NOT WHOLE DATA!!! 
                ### orignal line IS A MISTAKE
                """omit_obs = data.drop(
                    index = omit_index, 
                    axis = 0
                )"""
                omit_obs = data.iloc[b_index[i].index,:].drop(
                    index = omit_index, axis = 0) 
                """should be the correct code.
                    idea is that we take the subset of the array and 
                    return a correct subset. """
